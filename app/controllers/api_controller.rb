...
class ApiController < ApplicationController
  ...
  def rapid_questions
    ...
    year_first = Question.scope_zhenti.order('num asc').limit(1).first.num.to_s[0..3].to_i
    year_last = Question.scope_zhenti.order('num desc').limit(1).first.num.to_s[0..3].to_i
    year = (year_first..year_last).to_a.sample
    questions = Question.scope_zhenti.where("num like '#{year}#{volumn}%'").random(15)
    @content = wrap_questions(questions)
    render :json=>@content
  end
