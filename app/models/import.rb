...
class Import < ActiveRecord::Base
  ...
  validates :title,:presence=>true,:if=>"state == 'laws' || state == 'freelaws' || question_kind == 'scope_moni'"
  ...
  def import_question(data)
    ...
    if data && data[0][0..14] == %w(真题题号 标题 类型 分值 答案 解析一 解析三 选项A 解析A 选项B 解析B 选项C 解析C 选项D 解析D)
      ...
      data[1..-1].each do |row|
        if row[1]
          q = Question.find_or_initialize_by_num row[0].to_s.split('.')[0]
          p q.id
          q.kind = self.question_kind || 'zhenti'
          q.epmenu = self.title
          q.title = row[1].strip
          q.state = row[2]
          q.score = row[3]
          q.num = row[0].to_s.split('.')[0]
          q.title = row[1]
          q.answer = row[4]
          q.description = row[5]
